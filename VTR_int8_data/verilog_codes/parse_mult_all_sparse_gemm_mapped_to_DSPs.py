            ...
            if (line.find("""Block Type   # Blocks   Avg. # of input clocks and pins used   Avg. # of output pins used""")!= -1):
                ...
                for index, row in df_usage_pin_util.iterrows():
                    ...
                    if (row['Block Type'] == 'sparse_dense_ts_top'):
                        TSs = row['Usage']
                        TS_util = row['In_pin_utilization']

                    if (row['Block Type'] == 'dsp_top'):
                        DSPs = row['Usage']

                        # since each DSP can perform 4 9x9 multipliers (we have 8x8)
                        # for more accurate evaluation, we devide by 4
                        DSPs = int(DSPs / 4)
                ...
                # multiply the input utilization by 4 for DSPs
                total_inputs = CLBs*CLB_util + TSs*TS_util + BRAMs*BRAM_util + DSPs*4*DSP_util
