...
def gen_verilog_dense_gemm_sparse_TS(X, Y, K, U, V):

    print("# of Tensor Slices:", Y*X)
    print("# of BRAMs:", Y*X*4 + Y + X)

    with open("dense_gemm_sparse_TS.v", "w") as f:
...
input clk, reset, enable;""", file=f)
        ...
        for i in range(0, Y):
            print("wire [8-1:0] uncon_a0_" + str(i) + ";", file=f)

        print("", file=f)

        for i in range(0, Y):
            print("assign A_in0_" + str(i) + " = {uncon_a0_" + str(i) + ", A" + str(i) + "_data};", file=f)
        ...
        for i in range(0, X):
            print("wire [96-1:0] uncon_b" + str(i) + "_0;", file=f)

        print("", file=f)

        for i in range(0, X):
            print("assign B_in" + str(i) + "_0 = {uncon_b" + str(i) + "_0, B" + str(i) + "_data};", file=f)
        ...
        for y in range(0, Y):
            for x in range(0, X-1):
                print("wire [40-1:0] A_out" + str(x) + "_" + str(y) + ";", file=f)

        print("", file=f)        

        for y in range(0, Y):
            for x in range(0, X-1):
                print("wire [40-1:0] A_in" + str(x+1) + "_" + str(y) + ";", file=f)

        print("", file=f)        

        for y in range(0, Y):
            for x in range(0, X-1):
                print("wire [8-1:0] uncon_in" + str(x+1) + "_" + str(y) + ";", file=f)

        print("", file=f)        

        for y in range(0, Y):
            for x in range(0, X-1):
                print("assign A_in" + str(x+1) + "_" + str(y) + " = {uncon_in" + str(x+1) + "_" + str(y) + ", A_out" + str(x) + "_" + str(y) + "[31:0]};", file=f)
        ...
        for x in range(0, X):
            ...
            print("sparse_dense_init TS_" + str(x) + "_" + str(0) + " (.clk(clk), .reset(reset), .enable(enable), .accumulate(" + accumulate + "), .K_size(`K), .sparsity_level(`DENSE),", file=f)
            ...
            for y in range(1, Y-1):
                ...
                if (x == 0):
                    ...
                    a_data = "A_in" + str(x) + "_" + str(y)
                    

                if (x == X -1):
                    a_data_out = ""
                else:
                    a_data_out = "A_out" + str(x) + "_" + str(y)
                ...
                print("sparse_dense_middle TS_" + str(x) + "_" + str(y) + " (.clk(clk), .reset(reset), .enable(enable), .accumulate(" + accumulate + "), .K_size(`K), .sparsity_level(`DENSE),", file=f)
            ...
            if (x == 0):
                ...
                a_data = "A_in" + str(x) + "_" + str(Y-1)

            if (x == X -1):
                a_data_out = ""
            else:
                a_data_out = "A_out" + str(x) + "_" + str(Y-1)
            ...
            print("sparse_dense_last TS_" + str(x) + "_" + str(Y-1) + " (.clk(clk), .reset(reset), .enable(enable), .accumulate(" + accumulate + "), .K_size(`K), .sparsity_level(`DENSE),", file=f)
...
# gen_verilog_dense_gemm_sparse_TS(X, Y, K, U, V)