...
module sparse_dense_dedicated_links_tensor_slice(
	clk, 
	reset, 
	a_data,
	a_data_out,
...
input [4*4*`DWIDTH-1:0] b_ded_in;
output [4*`DWIDTH + 4*2-1:0] a_data_out;
output [4*4*`DWIDTH-1:0] b_ded_out;
input [11:0] K_size;
input [1:0] sparsity_level;
input [4:0] x_loc, y_loc;
output reg [4*`ACCWIDTH-1:0] c_data;
input enable, accumulate, acc_ded_in;
output reg valid_out;
output accumulate_out, acc_ded_out;
...
// for the first one only (y_loc == 0), get it from user (global routing)
// for the rest, get it via the dedicated links
wire accumulate_1_00;
assign accumulate_1_00 = (y_loc == 0)? accumulate : acc_ded_in;
...
assign acc_ded_out = accumulate_5_13_31_22;
...
assign wire_b0 = (y_loc == 0) ? b_data[4*`DWIDTH-1:0] : b_ded_in[4*`DWIDTH-1:0];
...
assign wire_b1 = (y_loc == 0) ? b1_d : b_ded_in[2*4*`DWIDTH-1:4*`DWIDTH];
...
assign wire_b2 = (y_loc == 0) ? b2_dd : b_ded_in[3*4*`DWIDTH-1:2*4*`DWIDTH];
...
assign wire_b3 = (y_loc == 0) ? b3_ddd : b_ded_in[4*4*`DWIDTH-1:3*4*`DWIDTH];
...
// PEs instantiation
PE PE_00(.clk(clk), .reset(reset), .accumulate(accumulate_1_00), .accumulate_out(accumulate_2_01_10), .IN_A(wire_a0), .IN_B(wire_b0), .weight_index(wire_weight_ind_0), .weight_index_out(out_weight_ind_00_in_weight_ind_01), .OUT_A(out_a00_in_a01), .OUT_B(out_b00_in_b10), .OUT_MAC(out_mac_00), .enable(enable & loc_tensor_load_en), .enable_B_regs(load_enable_B_regs), .sparse_2_4_select(second_reg_select_sparse_2_4), .d_type(d_type));

PE PE_01(.clk(clk), .reset(reset), .accumulate(accumulate_2_01_10), .accumulate_out(accumulate_3_02_20_11), .IN_A(out_a00_in_a01), .IN_B(wire_b1), .weight_index(out_weight_ind_00_in_weight_ind_01), .weight_index_out(out_weight_ind_01_in_weight_ind_02), .OUT_A(out_a01_in_a02), .OUT_B(out_b01_in_b11), .OUT_MAC(out_mac_01), .enable(enable & loc_tensor_load_en), .enable_B_regs(load_enable_B_regs), .sparse_2_4_select(second_reg_select_sparse_2_4), .d_type(d_type));
PE PE_10(.clk(clk), .reset(reset), .accumulate(accumulate_2_01_10), .accumulate_out(accumulate_3_02_20_11), .IN_A(wire_a1), .IN_B(out_b00_in_b10), .weight_index(wire_weight_ind_1), .weight_index_out(out_weight_ind_10_in_weight_ind_11), .OUT_A(out_a10_in_a11), .OUT_B(out_b10_in_b20), .OUT_MAC(out_mac_10), .enable(enable & loc_tensor_load_en), .enable_B_regs(load_enable_B_regs), .sparse_2_4_select(second_reg_select_sparse_2_4), .d_type(d_type));

PE PE_02(.clk(clk), .reset(reset), .accumulate(accumulate_3_02_20_11), .accumulate_out(accumulate_4_03_30_21_12), .IN_A(out_a01_in_a02), .IN_B(wire_b2), .weight_index(out_weight_ind_01_in_weight_ind_02), .weight_index_out(out_weight_ind_02_in_weight_ind_03), .OUT_A(out_a02_in_a03), .OUT_B(out_b02_in_b12), .OUT_MAC(out_mac_02), .enable(enable & loc_tensor_load_en), .enable_B_regs(load_enable_B_regs), .sparse_2_4_select(second_reg_select_sparse_2_4), .d_type(d_type));
PE PE_20(.clk(clk), .reset(reset), .accumulate(accumulate_3_02_20_11), .accumulate_out(accumulate_4_03_30_21_12), .IN_A(wire_a2), .IN_B(out_b10_in_b20), .weight_index(wire_weight_ind_2), .weight_index_out(out_weight_ind_20_in_weight_ind_21), .OUT_A(out_a20_in_a21), .OUT_B(out_b20_in_b30), .OUT_MAC(out_mac_20), .enable(enable & loc_tensor_load_en), .enable_B_regs(load_enable_B_regs), .sparse_2_4_select(second_reg_select_sparse_2_4), .d_type(d_type));
PE PE_11(.clk(clk), .reset(reset), .accumulate(accumulate_3_02_20_11), .accumulate_out(accumulate_4_03_30_21_12), .IN_A(out_a10_in_a11), .IN_B(out_b01_in_b11), .weight_index(out_weight_ind_10_in_weight_ind_11), .weight_index_out(out_weight_ind_11_in_weight_ind_12), .OUT_A(out_a11_in_a12), .OUT_B(out_b11_in_b21), .OUT_MAC(out_mac_11), .enable(enable & loc_tensor_load_en), .enable_B_regs(load_enable_B_regs), .sparse_2_4_select(second_reg_select_sparse_2_4), .d_type(d_type));

PE PE_03(.clk(clk), .reset(reset), .accumulate(accumulate_4_03_30_21_12), .accumulate_out(accumulate_5_13_31_22), .IN_A(out_a02_in_a03), .IN_B(wire_b3), .weight_index(out_weight_ind_02_in_weight_ind_03), .weight_index_out(a_data_out[4*`DWIDTH + 1:4*`DWIDTH]), .OUT_A(a_data_out[`DWIDTH-1:0]), .OUT_B(out_b03_in_b13), .OUT_MAC(out_mac_03), .enable(enable & loc_tensor_load_en), .enable_B_regs(load_enable_B_regs), .sparse_2_4_select(second_reg_select_sparse_2_4), .d_type(d_type));
PE PE_30(.clk(clk), .reset(reset), .accumulate(accumulate_4_03_30_21_12), .accumulate_out(accumulate_5_13_31_22), .IN_A(wire_a3), .IN_B(out_b20_in_b30), .weight_index(wire_weight_ind_3), .weight_index_out(out_weight_ind_30_in_weight_ind_31), .OUT_A(out_a30_in_a31), .OUT_B(b_ded_out[4*`DWIDTH-1:0]), .OUT_MAC(out_mac_30), .enable(enable & loc_tensor_load_en), .enable_B_regs(load_enable_B_regs), .sparse_2_4_select(second_reg_select_sparse_2_4), .d_type(d_type));
PE PE_21(.clk(clk), .reset(reset), .accumulate(accumulate_4_03_30_21_12), .accumulate_out(accumulate_5_13_31_22), .IN_A(out_a20_in_a21), .IN_B(out_b11_in_b21), .weight_index(out_weight_ind_20_in_weight_ind_21), .weight_index_out(out_weight_ind_21_in_weight_ind_22), .OUT_A(out_a21_in_a22), .OUT_B(out_b21_in_b31), .OUT_MAC(out_mac_21), .enable(enable & loc_tensor_load_en), .enable_B_regs(load_enable_B_regs), .sparse_2_4_select(second_reg_select_sparse_2_4), .d_type(d_type));
PE PE_12(.clk(clk), .reset(reset), .accumulate(accumulate_4_03_30_21_12), .accumulate_out(accumulate_5_13_31_22), .IN_A(out_a11_in_a12), .IN_B(out_b02_in_b12), .weight_index(out_weight_ind_11_in_weight_ind_12), .weight_index_out(out_weight_ind_12_in_weight_ind_13), .OUT_A(out_a12_in_a13), .OUT_B(out_b12_in_b22), .OUT_MAC(out_mac_12), .enable(enable & loc_tensor_load_en), .enable_B_regs(load_enable_B_regs), .sparse_2_4_select(second_reg_select_sparse_2_4), .d_type(d_type));
...
PE PE_32(.clk(clk), .reset(reset), .accumulate(accumulate_6_23_32), .accumulate_out(accumulate_7_33), .IN_A(out_a31_in_a32), .IN_B(out_b22_in_b32), .weight_index(out_weight_ind_31_in_weight_ind_32), .weight_index_out(out_weight_ind_32_in_weight_ind_33), .OUT_A(out_a32_in_a33), .OUT_B(b_ded_out[3*4*`DWIDTH-1:2*4*`DWIDTH]), .OUT_MAC(out_mac_32), .enable(enable & loc_tensor_load_en), .enable_B_regs(load_enable_B_regs), .sparse_2_4_select(second_reg_select_sparse_2_4), .d_type(d_type));

PE PE_33(.clk(clk), .reset(reset), .accumulate(accumulate_7_33), .accumulate_out(), .IN_A(out_a32_in_a33), .IN_B(out_b23_in_b33), .weight_index(out_weight_ind_32_in_weight_ind_33), .weight_index_out(a_data_out[4*`DWIDTH + 7:4*`DWIDTH + 6]), .OUT_A(a_data_out[4*`DWIDTH-1:3*`DWIDTH]), .OUT_B(b_ded_out[4*4*`DWIDTH-1:3*4*`DWIDTH]), .OUT_MAC(out_mac_33), .enable(enable & loc_tensor_load_en), .enable_B_regs(load_enable_B_regs), .sparse_2_4_select(second_reg_select_sparse_2_4), .d_type(d_type));
...
//////// PE ///////////////////////////////////
///////////////////////////////////////////////
module PE(clk, reset, accumulate, accumulate_out, IN_A, IN_B, weight_index, weight_index_out, OUT_A, OUT_B, OUT_MAC, enable, enable_B_regs, sparse_2_4_select, d_type);

input accumulate, enable, enable_B_regs, sparse_2_4_select, d_type;
input clk, reset;
...
MAC_unit_3stages MAC_inst (.clk(clk), .reset(reset), .accumulate(accumulate), .IN_A(IN_A), .IN_B(MUX_OUT), .OUT_C(OUT_MAC), .enable(enable), .d_type(d_type));
